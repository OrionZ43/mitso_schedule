...
подгрупп, страницы подробностей, сохранённых справок и настроек — в `docs/screenshots/`. Фото справки
...
Рисует слои значков и кладёт их в `android/app/src/main/res/mipmap-*`, образцы для настроек —
в `assets/app_icons/`, витрину — в `docs/icon/`. Логотип — шапка выпускника (`Symbols.school`)
в форме `MaterialShapes.cookie9Sided`, той же, что у номера аудитории на карточке пары; фон —
`primary` эталонной схемы M3.
...
и монохромный для тем Android 13+ (силуэт с вырезанным значком, система красит его сама);
для Android ниже 26 генерируются обычные PNG.

**Значок выбирается в настройках**: «Печенька и шапка», «Шапка», «Печенька и часы», «Карточка и
шапка» — на случай, если лаунчер сам обрезает значки фигурой и печенька в печеньке выглядит
лишней. Каждый вариант — свой `activity-alias` в манифесте со своими слоями; приложение включает
нужный и выключает остальные через `PackageManager.setComponentEnabledSetting` (канал
`mitso/app_icon`, `MainActivity.kt`) — другого способа поменять значок у Android нет. Лаунчер
обновляет значок сам, обычно за пару секунд. В `docs/icon/` лежат и отвергнутые варианты — волна,
лента дней, буква.
...
Задачи пользователя: что сделать, по какому предмету и к какому сроку. Список разложен по
срокам — «Просрочено», «Сегодня», «Завтра», «На этой неделе», «Позже», «Без срока»; выполненные
уходят на отдельную вкладку. Срок выбирается чипами «Сегодня / Завтра / дата», предмет —
чипом из загруженного расписания. Нажатие на задачу открывает правку, там же удаление.
Всё хранится в `shared_preferences` (`tasks.items`).

## Лицевой счёт: student.mitso.by

Профиль показывает данные личного кабинета. Сайт простой: `POST login_stud.php` с полями
`login` и `password` — оба равны номеру лицевого счёта — сразу отдаёт страницу, сессия не нужна.
Со страницы берутся ФИО, баланс с датой («на конец дня»), основной долг, пеня и доступ к СДО
(группа, логин, пароль). Пропусков там нет.

- **Номер счёта вводит пользователь** и может отключить счёт в настройках: тогда номер, баланс и
  доступ к СДО стираются с телефона. Данные лежат в `shared_preferences` приложения.
- **Обновление.** Сайт пересчитывает счёт раз в сутки в 13:00, поэтому приложение запрашивает его,
  только если сохранённые данные старее последних 13:00: при открытии профиля, при возвращении в
  приложение и в фоне раз в шесть часов (`workmanager`, только при наличии сети).
- **Уведомление о долге** (`flutter_local_notifications`): приходит, когда долг появился или вырос,
  и не повторяется на том же долге; когда долг закрыт, уведомление снимается. Android 13+ спрашивает
  разрешение при включении настройки. Выключается там же, в настройках.
- **СДО** — `https://sdo.mitso.by`. Кнопка «Войти в СДО» копирует пароль в буфер и открывает сайт:
  форма входа Moodle не принимает данные ссылкой. Пароль в профиле скрыт, показывается по нажатию.
  Глубже в Moodle приложение не лезет: мобильный API там включён, но заходят в СДО пару раз в
  месяц — курсы и дедлайны в приложении не нужны.
- **Баланс** показан в BYN; при долге карточка целиком переходит на роли `error`.
...
| Диалог подтверждения, поле ввода | `AlertDialog`, `TextField` Flutter M3 | — |
...
21. **Пары — карточками, а не списком** (решение заказчика): гайд cards → Adaptive советует
    ...
    угла карточки (гайд shape: абстрактные формы — для декора, не для текста); форма очень
    медленно вращается (90 с на оборот) — украшение по просьбе заказчика, при «Удалить анимации»
    стоит.
22. **Поля ввода и выбор даты** — outlined text field и `showDatePicker` Flutter M3, без порта
    из Compose: их в приложении мало.
23. **Анимация пустого состояния придумана в рамках гайда**: сами циклы форм, периоды вращения
    (кратные 4666 мс из `LoadingIndicator`), такт морфа 1000 мс и сдвиги тактов слоёв —
    наш выбор; гайд даёт только приёмы (морф, вращение, пружины) и запрет на декор при
    «Удалить анимации». Столкновений фигур нет: в M3 «bounce» — это перелёт пружины,
    физики тел в спеке нет вовсе.
...
lib/
  ...
  features/                  boot, home, schedule (карточки пар, неделя, поиск), group_picker,
                             absences, notes, profile (лицевой счёт, СДО), settings
