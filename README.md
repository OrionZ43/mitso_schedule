...
- **Подпись.** Манифест принимается, только если подпись сходится с публичным ключом из
  `lib/features/updater/update_keys.dart`. **Пока карта ключей пуста, обновления выключены
  совсем: приложение не ходит в сеть и пишет об этом в настройках.** Подпись проверяется по
  сырым байтам `payload`, поэтому канонизация JSON не нужна.
...
dart run tool/update_signing.dart keygen z43-2026 D:/keys/mitso-update.key
...
dart run tool/update_signing.dart sign z43-2026 D:/keys/mitso-update.key manifest.json mitso-update.json
```
