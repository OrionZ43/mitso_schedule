...
- Виджет: `M3IconButton` (`lib/widgets/m3_buttons.dart`) на ядре `M3ButtonContainer` (см. `buttons.md`). Стили filled (по умолчанию) / tonal / outlined / standard (`M3IconButtonColor`), размеры XS–XL (`M3ButtonSize`), ширины narrow / uniform / wide (`M3IconButtonWidth`), форма round / square. Toggle — `isSelected` + `selectedIcon` (outlined-иконка в `icon`, filled — в `selectedIcon`). Подсказка — параметр `tooltip`, пока обычный Flutter `Tooltip`.
- Точно: контейнер = `IconButtonDefaults.<size>ContainerSize(widthOption)` (иконка + 2 × отступ по таблице, высота размера); иконки 20 / 24 / 24 / 32 / 40dp; pressed и square-радиусы по токенам; toggle: выбранная round → `selected.container.shape.round`, square → full (`toggleableShapes`); морф и у default, и у toggle — DefaultEffects (`shapeForInteraction`); цвета по таблице выше, цвет без анимации; outlined-обводка по размеру, у выбранной обводки нет; disabled — onSurface 38% / контейнер 10%; зона нажатия 48dp; семантика toggle — checked (`Role.Checkbox`).
- Отступления: standard и outlined по умолчанию берут цвета токенов m3 (onSurfaceVariant, outlineVariant) — Compose `iconButtonColors()` берёт `LocalContentColor`, токены даёт только `*VibrantColors()`. API toggle — как у Flutter `IconButton(isSelected, onPressed)`, а не `onCheckedChange`.
- Экраны пока на `IconButton.filledTonal` / `IconButton` с `AppButtonStyles.iconSmall`; перевод на `M3IconButton` — отдельный шаг.
