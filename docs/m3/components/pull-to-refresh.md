...
Статус в приложении: ✅ соответствует — порт `PullToRefresh.kt`, есть кнопка «Обновить» (отступления — README, п. 8)
...
`lib/widgets/m3_pull_to_refresh.dart` перенесён на `PullToRefreshModifierNode` и
`PullToRefreshDefaults.LoadingIndicator`. Строки таблицы «Расхождения» ниже с номерами 2–6 и
8–12 закрыты. Строка 7 закрыта в виджете: появился параметр `edgeOffset`, но экран должен его
передать. Строка 1 (кнопка «Обновить» в app bar) — задача экрана. Для неё есть параметр
`isRefreshing`.

**API:** `M3PullToRefresh(onRefresh, child, isRefreshing, edgeOffset, semanticsLabel)`.
Индикатор держится на пороге, пока не завершился `Future` из `onRefresh` или пока
`isRefreshing == true`.

**Совпадает с Compose**
- Жест:
  - порог 80dp, множитель 0,5;
  - смещение считает `calculateVerticalOffset`: линейно до порога, дальше `t − t²/4`,
    не больше двух порогов;
  - `distanceFraction = verticalOffset / 80`, сверху не ограничена.
- Движение вверх, пока индикатор вытянут, сначала сворачивает протяжку и только потом
  прокручивает список (`onPreScroll`). Остаток, который список не прокрутил, уходит в
  протяжку (`onPostScroll`). Stretch-эффекта при этом нет, потому что смещение поглощено.
- Протяжка не принимается, пока идёт анимация положения (`state.isAnimating`) и пока идёт
  обновление.
- Отпускание: обновление запускается, только если протяжка строго больше порога. Иначе
  индикатор уходит.
- Положение анимирует `spring()`: 1.0 / 1500, порог 0.01, завершение по оценке Compose. По
  окончании (или отмене) выполняются `finally` из `animateToThreshold` / `animateToHidden`.
- Индикатор: бокс 48dp, круг `primaryContainer`, без тени (Level0), без масштаба.
  - Положение: `translateY = fraction × 80 − 48`, с обрезкой по кромке (`edgeOffset`).
  - Пока тянут — `M3LoadingIndicator.determinate(progress: fraction, contained: true)`; после
    порога весь компонент дополнительно поворачивается на `−(p − 1) × 180°`.
  - Во время обновления — неопределённый индикатор. Переход между ними — кросс-фейд пружиной
    DefaultEffects (1.0 / 1600).
- Семантика: при протяжке `progressBar` со значением, при обновлении `loadingSpinner`. Подпись
  по умолчанию «Обновление расписания».

**Уменьшение движения.** Пружины положения и кросс-фейд завершаются сразу, как в Compose при
`MotionDurationScale = 0`. Следование за пальцем остаётся: это прямое управление.

**Отступления**
- Во Flutter нет вложенной прокрутки. Её место занимает `ScrollPhysics.applyPhysicsToUserOffset`:
  физика подмешивается через `ScrollConfiguration` всем вертикальным спискам внутри `child`.
  Отсюда два условия:
  - физика списка должна передавать вызов родителю. `AlwaysScrollableScrollPhysics` и
    `ClampingScrollPhysics` это делают, `BouncingScrollPhysics` — нет;
  - короткий список должен быть `AlwaysScrollable`.
  Сколько прокрутит сам список, считается по правилам «зажатой» прокрутки.
- Момент отпускания (`onPreFling`) ловит `Listener`: срабатывает, когда поднят последний палец.
- Если протяжка отпущена за порогом, индикатор сразу едет к порогу. В Compose сначала
  стартует `animateToHidden`, а к порогу он поворачивает на следующем кадре, после того как
  приложение выставит `isRefreshing`. Разница — один кадр.
- Спрятанный индикатор (`fraction == 0`, обновления нет) убирается из дерева: без тикера и без
  узла семантики. В Compose он остаётся в композиции, но целиком обрезан.
- Снизу область обрезается по нижнему краю виджета, а не уходит в бесконечность. При высоте
  области меньше 160dp + `edgeOffset` сильно вытянутый индикатор обрежется снизу.
- Тест «повторная протяжка во время отката» переписан под правило `isAnimating`. Во время
  отката (~130–190 мс) протяжка не принимается. Когда откат закончился, индикатор снова
  следует за пальцем, и устаревшее завершение отката его не прячет.
