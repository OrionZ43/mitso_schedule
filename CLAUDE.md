...
4. Каждое отступление от спеки — осознанное: записать причину в `docs/m3/`
   и в раздел «Сознательные отступления» [`docs/development.md`](docs/development.md).
...
flutter build apk --release -t lib/main.dart     # -t обязателен, см. docs/development.md
...
отрисовки за кадр, см. `docs/development.md` → «Производительность»).
...
- student.mitso.by (лицевой счёт) — только с согласия и данными пользователя: номер вводится в
  приложении, запрос идёт при открытии профиля и в фоне не чаще раза в шесть часов (сайт
  пересчитывает счёт раз в сутки в 13:00). Настоящие номера, ФИО и пароли СДО в репозиторий не
  попадают: в тестах — выдуманные (`test/fixtures/student_account.html`).
